...
def merge_table(table_left, table_right, column):
    ...
    right_width = table_right.index.shape[0]
    ...
    for i, column in enumerate(all_columns):
        if column in table_left.keys and column in table_right.keys:
            ...
            left_data[left_index.astype(np.bool)] = table_left.data[left_key]
            right_data[right_index.astype(np.bool)] = table_right.data[right_key]
